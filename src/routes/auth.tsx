...
function AuthPage() {
  ...
  if (user) router.navigate({ to: search.redirect as any });
    ...
    try {
      ...
      if (mode === "signup") {
        const { error } = await supabase.auth.signUp({
          ...
          options: { data: { full_name: form.fullName }, emailRedirectTo: `${window.location.origin}/portal/patient` },
        });
        if (error) throw error;
        toast.success("Account created! Welcome to Oakwood.");
  ...
  const handleGoogle = async () => {
    setLoading(true);
    const res = await lovable.auth.signInWithOAuth("google", { redirect_uri: window.location.origin + (search.redirect as string) });
          ...
          <div className="p-8">
            <Tabs value={mode} onValueChange={(v) => setMode(v as any)}>
              ...
              <form onSubmit={handleEmail} className="space-y-3">
                {mode === "signup" && (
                  <div><Label htmlFor="fn">Full name</Label><Input id="fn" value={form.fullName} onChange={e => setForm({ ...form, fullName: e.target.value })} required /></div>
                )}
                ...
                <div><Label htmlFor="pw">Password</Label><Input id="pw" type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} required minLength={8} /></div>
