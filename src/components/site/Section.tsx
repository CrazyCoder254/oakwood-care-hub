...
export function SectionHeader({ eyebrow, title, subtitle, center = true }: { eyebrow?: string; title: string; subtitle?: string; center?: boolean }) {
  return (
    <div className={center ? "text-center max-w-2xl mx-auto mb-12" : "max-w-2xl mb-10"}>
      {eyebrow && <span className="text-xs tracking-[0.25em] uppercase text-primary/70 font-medium">{eyebrow}</span>}
  ...
  return (
    <section className="gradient-hero text-primary-foreground">
      <div className="container mx-auto px-4 py-20 md:py-28 text-center">
        {eyebrow && <span className="text-xs tracking-[0.3em] uppercase text-primary-foreground/80">{eyebrow}</span>}
        <h1 className="font-display text-4xl md:text-6xl text-primary-foreground mt-3">{title}</h1>
